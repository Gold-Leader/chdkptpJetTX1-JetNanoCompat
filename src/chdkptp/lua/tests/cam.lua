...
-- setup functions
local function ensure_connected(self)
	self.step_opts_run.setup.fail_abort_all = true
	if con:is_connected() then
		return
	end
	local devspec = m.connect_devspec
	local cmd = 'connect'
	if devspec then
		cmd = cmd .. ' ' .. devspec
	end
...
local function ensure_connected_rec(self)
