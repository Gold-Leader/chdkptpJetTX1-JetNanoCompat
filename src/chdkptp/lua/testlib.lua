...
function test_methods:do_step(step,opts)
	...
	local status,fstatus = xpcall(self[step],errutil.format_traceback,self)
...
function m.assert_thrown(f,match)
	...
	if type(err) ~= 'table' then
		error('expected errlib object',2)
	end

	if match.etype then
		if match.etype ~= err.etype then
			error('expected etype='..tostring(match.etype),2)
		end
	end
